...
def read_files(data):
    ...
    with open(fileIn) as file:
        ...
        for line in file:
            ...
            if index[2] == '-1':
                ...
                indexQ.append(index[0]) #MS
                indexQ.append(index[1]) #MS
                indexQ.append(index[2]) #MS
                lookupProvider.append(indexQ) #MS
            
            # peers - p2p-link
            elif index[2] == '0':
                data[index[0]]['as_links']['p2p'].append(index[1])
    file.close()
    print ("Print lookupProvider") #MS
    ...
    for a_s in data: #MS
        ...
        for x in lookupProvider: #MS
            ...
            if a_s == x[1]: #MS
                data[a_s]['as_links']['pro'].append(x[0]) #MS


    print ("Back to Thao code")
...
def write_new_file(data, t1):
    ...
    with open(output_data_file, "w+") as w:
        ...
        for a_s in data:
            # no. of customers
            p2c_len = len(data[a_s]['as_links']['p2c']) # MS
            # no. of peers
            p2p_len = len(data[a_s]['as_links']['p2p']) # MS
            # no. of providers
            pro_ln = len(data[a_s]['as_links']['pro']) # MS
            # degrees
            deg = data[a_s]['degree'] # MS
            # prefixes
            pre = data[a_s]['prefix'] # MS
            # length
            d_len = data[a_s]['length'] # MS
            
            print('AS: {} | n_node_degree: {} | IP_prefix: {} | customers: {} | peers: {} | providers: {}'.format(a_s, deg, pre + '/' + d_len, p2c_len, p2p_len, pro_ln), file=w) # MS
    w.close()
