        ...
        for line in file:
            ...
            for a_s in prefix_as:
                ...
                if ',' in a_s:
                    ...
                    for sub_as1 in set_as1:
                        if sub_as1 not in data:
                            data[sub_as1] = {'as_links': {'p2c': [],
                                                    'p2p': [],
                                                    'prov': []
                                                    },
                                                    'degree': 0,
                                                    ...
                                                    'prefix': [],
                                                    'length': 'NA'}
                    if not data[sub_as1]['prefix'].count(index[0] + '/' + index[1]):
                        data[sub_as1]['prefix'].append(index[0] + '/' + index[1])
...
def write_new_file(data, t1, n_prefix):
    ...
    with open(output_data_file, "w+") as w:
        ...
        print('AS | Node_degree | Prefix Count | AS Class', file=w)
        ...
        for a_s in data:
            ...
            print('AS: {} | Node Degree: {} | IP Prefix: {} | AS Class: {} | Customers: {} | Peers: {} | Providers: {}'.format(a_s, deg, pre, d_class, p2c_len, p2p_len, prov_len), file=w)
...
def n_degree(data):
    for a_s in data:
        data[a_s]['degree'] = len(data[a_s]['as_links']['p2c']) + len(data[a_s]['as_links']['p2p'])+len(data[a_s]['as_links']['prov'])
